import { deleteNote, NOTES } from "../data/notes.js";
...
export const fillTable = () =>{
    ...
    const edit_btns = document.querySelectorAll('[data-btn="edit"]');

    edit_btns.forEach(edit => edit.addEventListener("click", edit))
}
...
const makeRow = (note) =>{
    ...
    row.appendChild(makeActionBtns(note.id))
    return row
}
...
const noteToRowHTML = (note) => {
    return Object.keys(note).slice(2).reduce((tds, key) => 
        tds += key == 'created' ? 
            `<td data-key=${key}>${new Date(note[key]).toLocaleDateString('en-US')}</td>`
            : `<td data-key=${key}>${note[key]}</td>`, 
...
const makeActionBtns = (id) => {
    ...
    td.appendChild(createActionBtn('edit', editSVG, editRow))
    td.appendChild(createActionBtn('delete', deleteSVG, deleteRow))
    return td
}
